...
export const Sales = () => {
   ...
   const getSales = async () => {
      const res = await axios.get(`${url}sales2024`);
      setSales(res.data);

      const se = await axios(`${url}salesEvent2024`);
      setEvent(se.data);
      ...
      const wtp = await axios(`${url}waiterToPay2024`);
   ...
   const updateWaiterPay = () => {
      if(date.length > 10) show_alerta('La Fecha es incorrecta, seleccione de nuevo', 'warning');
         ...
         axios.put(`${url}paymentWaiterUpdated2024/${ids}`, {
            idPaymentWaiter: ids,
            idSales: idSales,
            date: date,
            time: time,
   ...
   const modalInfo = async (idSales, description, customer, date, time, amount, price, total, comment) => {
      ...
      const pw = await axios(`${url}paymentWaiter2024/${idSales}`);
      setPaymentWaiter(pw.data);

      const pwt = await axios(`${url}paymentWaiterTotal2024/${idSales}`);
      ...
      if (op === 1) {
         if (description === '') show_alerta('Escribe la descripcion de la venta', 'warning')
            ...
            if (operation == 1) {
               ...
               axios.put(`${url}salesUpdate2024/${ids}`, {
                  idSales: ids,
         ...
         if (idTeamMember === undefined) show_alerta('Seleccione El Camarero', 'warning')
            ...
            axios.post(`${url}paymentWaiter2024`, {
   ...
   const deleteSales = (id, description) => {
      ...
      MySwal.fire({
         ...
         if (result.isConfirmed) {
            axios.delete(`${url}salesDelete2024/${id}`);
   ...
   const deleteWaiter = (id, lastName, firstName, descripcion) => {
      ...
      MySwal.fire({
         ...
         if (result.isConfirmed) {
            axios.delete(`${url}paymentWaiterDelete2024/${id}`);
